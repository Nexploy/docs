...
/**
 * Traductions de l'interface Fumadocs.
 * L'anglais est la langue par défaut fournie par `uiTranslations()` ;
 * on ajoute la traduction française par-dessus.
 */
...
/**
 * Shared layout options (nav bar, links, logo) used across layouts.
 * `lang` is the active locale, used to prefix internal links.
 */
export function baseOptions(lang: string): BaseLayoutProps {
  return {
    // Active le sélecteur de langue dans la barre de navigation.
    i18n: true,
    nav: {
      title: (
        <>
          ...
          />
          <span className="font-semibold">Nexploy</span>
        </>
      ),
      url: lang === 'fr' ? '/' : `/${lang}`,
    },
    // Icône GitHub intégrée, affichée en bas de la barre latérale.
    githubUrl: 'https://github.com/Nexploy',
    // Lien « Site » sous forme d'icône, à côté de GitHub (secondary par défaut).
    links: [
      {
        type: 'icon',
        icon: <Globe />,
